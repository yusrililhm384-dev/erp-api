...
type Service interface {
	List(ctx context.Context, page uint) (*readmodel.StatusListResponse, error)
	Detail(ctx context.Context, statusId uint) (*readmodel.StatusDetail, error)
	Create(ctx context.Context, userId uint, req *dto.CreateStatusReq) error
	Update(ctx context.Context, userId uint, statusId uint, req *dto.UpdateStatusReq) error
	Delete(ctx context.Context, userId uint, statusId uint) error
}
...
func (h *Handler) List(c *echo.Context) error {
	...
	return c.JSON(http.StatusOK, &response.Response{
		...
		Message: "Status list successfully fetched!",
...
func (h *Handler) Detail(c *echo.Context) error {
	c.Logger().Info("Status Detail")

	statusId, err := strconv.ParseUint(c.Param("statusId"), 10, 64)
	...
	data, err := h.service.Detail(c.Request().Context(), uint(statusId))
	...
	return c.JSON(http.StatusOK, &response.Response{
		Success: true,
		Status:  http.StatusOK,
		Message: "Status detail successfully fetched!",
		Data:    data,
	})
}

func (h *Handler) Create(c *echo.Context) error {
	c.Logger().Info("Create Status")
...
func (h *Handler) Update(c *echo.Context) error {
	...
	if err := h.service.Update(c.Request().Context(), sessionData.UserId, uint(statusId), req); err != nil {
		c.Logger().Warn(err.Error())
...
func (h *Handler) Delete(c *echo.Context) error {
	...
	if err := h.service.Delete(c.Request().Context(), sessionData.UserId, uint(statusId)); err != nil {
		c.Logger().Warn(err.Error())

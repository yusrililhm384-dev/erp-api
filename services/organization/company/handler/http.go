...
type Service interface {
	List(ctx context.Context, page uint) (*readmodel.CompanyListResponse, error)
	Detail(ctx context.Context, companyId uint) (*readmodel.CompanyDetail, error)
	Create(ctx context.Context, userId uint, req *dto.CreateCompanyReq) error
	Update(ctx context.Context, userId uint, companyId uint, req *dto.UpdateCompanyReq) error
	Delete(ctx context.Context, userId uint, companyId uint) error
}
...
func (h *Handler) List(c *echo.Context) error {
	...
	return c.JSON(http.StatusOK, &response.Response{
		...
		Message: "Company list successfully fetched!",
...
func (h *Handler) Detail(c *echo.Context) error {
	c.Logger().Info("Company Detail")

	companyId, err := strconv.ParseUint(c.Param("companyId"), 10, 64)
	...
	data, err := h.service.Detail(c.Request().Context(), uint(companyId))
	...
	return c.JSON(http.StatusOK, &response.Response{
		Success: true,
		Status:  http.StatusOK,
		Message: "Company detail successfully fetched!",
		Data:    data,
	})
}

func (h *Handler) Create(c *echo.Context) error {
	c.Logger().Info("Create Company")
...
func (h *Handler) Update(c *echo.Context) error {
	...
	if err := h.service.Update(c.Request().Context(), sessionData.UserId, uint(companyId), req); err != nil {
		c.Logger().Warn(err.Error())
...
func (h *Handler) Delete(c *echo.Context) error {
	...
	if err := h.service.Delete(c.Request().Context(), sessionData.UserId, uint(companyId)); err != nil {
		c.Logger().Warn(err.Error())
